...
function profileToText(
  ...
  const profileFields = [profile.personality_type, profile.idea_status, profile.desired_role_in_team, profile.self_introduction_comment].filter(Boolean);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Parsed userId:", userId, "desiredRole:", desiredRole, "mode:", mode, "useVectorMatching:", useVectorMatching);
    ...
    if (mode === "matching") {
      ...
      if (!shouldUseVectorMatching) {
        ...
        return await handleFlexibleMatching(userId, desiredRole);
      }
...
async function handleFlexibleMatching(userId: number, desiredRole: string | null) {
  ...
  try {
    ...
    // 🎯 修正: 候補者取得でフレキシブル検索対応
    const whereCondition: any = { id: { not: userId } };
    if (desiredRole) {
    ...
    // 🎯 マッチングスコア計算
    const matchResults = filteredCandidates.map((candidate) => {
