...
export default function ProfileSetupScreen({ onNavigate }: ProfileSetupScreenProps) {
  ...
  const teamRoles = [
    ...
    { value: "tech_lead", label: "Techリード (技術選定・アーキテクチャ設計など技術面を牽引)" },
    { value: "developer_main", label: "開発メイン (コーディング・実装に集中)" },
    { value: "biz_planning", label: "Biz・企画 (アイデア出し、ビジネスモデル検討、ユーザーリサーチなど)" },
    { value: "design_ux", label: "Design・UX (UIデザイン、ユーザー体験設計など)" },
    { value: "pm_management", label: "PM・進行管理 (タスク管理、スケジュール調整、コミュニケーション促進など)" },
    { value: "support_member", label: "メンバーとして幅広くサポート・貢献" },
  ]
  ...
  // 時間帯を平日・土日祝で分ける
  const weekdayTimeslots = masterData.timeslots
    .filter((t) => t.day_type === "weekday")
    .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0))
  const weekendTimeslots = masterData.timeslots
    .filter((t) => t.day_type === "weekend_holiday")
    .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0))
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          <Card className="border border-gray-200 shadow-sm">
            ...
            <CardContent>
              <RadioGroup
                ...
                onValueChange={(value) => setProfile({ ...profile, idea_status: value })}
                className="space-y-3"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {masterData.productGenres.map((genre) => (
                  <div key={genre.id} className="flex items-center space-x-2">
                    <Checkbox
                      id={`genre-${genre.id}`}
                      checked={profile.product_genre_ids.includes(genre.id)}
                      onCheckedChange={() => toggleProductGenre(genre.id)}
                    />
                  ...
                  {weekdayTimeslots.map((timeslot) => (
                    <div key={timeslot.id} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        checked={profile.timeslot_ids.includes(timeslot.id)}
          ...
          <Card className="border border-gray-200 shadow-sm">
            ...
            <CardContent>
              <RadioGroup
                value={profile.desired_role_in_team}
                onValueChange={(value) => setProfile({ ...profile, desired_role_in_team: value })}
                className="space-y-3"
              >
                {teamRoles.map((role) => (
                ...
                {masterData.teamPriorities.map((priority) => (
                  <div key={priority.id} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      checked={profile.team_priority_ids.includes(priority.id)}
                      onCheckedChange={() => toggleTeamPriority(priority.id)}
                    />
          ...
          <Card className="border border-gray-200 shadow-sm">
            ...
            <CardContent>
              ...
              <div className="text-right text-sm text-[#6C757D] mt-1">
                {profile.self_introduction_comment.length}/1000文字
              </div>
            </CardContent>
          </Card>
          ...
          <Button
            onClick={handleSave}
            disabled={isLoading}
            className="w-full h-12 bg-[#5D70F7] hover:bg-[#4D60E7] text-white font-medium text-lg disabled:opacity-50"
          >
